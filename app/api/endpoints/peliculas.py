...
async def crear_pelicula(pelicula: PeliculaCreate, db: Session = Depends(get_db)):
    try:
...
@router.get("/peliculas/", response_model=List[Pelicula], summary="Listar todas las películas")
...
@router.get("/peliculas/{id}", response_model=Pelicula, summary="Obtener película por ID")
async def obtener_pelicula(id: int, db: Session = Depends(get_db)):
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error al obtener película: {str(e)}")
